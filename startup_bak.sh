...
if [ "$CURRENT_USER" = "root" ]; then
    ...
    if [ -d "/workspace/.claude-persistent" ]; then
        ...
        echo "WARNING: No Claude persistent storage found at /workspace/.claude-persistent"
        echo "Claude will use ephemeral storage - data will be lost on restart!"
    fi
...
if [ "$CURRENT_USER" = "root" ] && [ "$TARGET_USER" = "anton" ]; then
    ...
    exec su - anton -c 'export NVM_DIR=/workspace/.nvm; . "$NVM_DIR/nvm.sh"; export PYTHONPATH=/workspace/diffing-toolkit/.local:$PYTHONPATH; export PIP_TARGET=/workspace/diffing-toolkit/.local; export PATH=/workspace/diffing-toolkit/.local/bin:$PATH; [ -f /workspace/.hf_token ] && export HF_TOKEN=$(cat /workspace/.hf_token); cd /workspace/diffing-toolkit && echo "Environment setup complete!" && echo "Python packages: /workspace/diffing-toolkit/.local" && echo "HF token loaded: $([ -f /workspace/.hf_token ] && echo Yes || echo No)" && exec bash'
else
